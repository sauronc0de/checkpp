...
source_dir="${PROJECT_ROOT}/sw_example/example_c"
...
usage() {
  cat <<EOF
SYNOPSIS
    ${SCRIPT_NAME} [--help|-h] [--version|-v] [--verbose] [--source_dir DIR] [--build_dir DIR]
DESCRIPTION
...
IMPLEMENTATION
    ...
    project         checkpp
    location        tools/scripts/checpp_c_simulation.sh
EOF
}

short_help() {
  printf '%s\n' "Rebuild/run the C example and check it with C-family rules; example: ${SCRIPT_NAME} --verbose; deps: cmake, checkpp."
}
...
app_path="${build_dir}/my_app"
if [ ! -x "${app_path}" ]; then
  printf 'Built application is missing or not executable: %s\n' "${app_path}" >&2
  exit 1
fi
...
log "Running checkpp"
"${checkpp_bin}" "${source_dir}" "${build_dir}" "${rules_file}" --ignore-paths "${ignore_paths}"
